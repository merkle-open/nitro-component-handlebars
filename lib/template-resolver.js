...
/**
 * If an array is expected a primitive is turned into an array
 * {{component 'demo' colors="red"}} is the same as {{component 'demo' colors="['red']"}}
 * @param {Object} renderData the template render data
 * @param {Object} attributeTypes the schema definition of the attributes
 * @returns {Object} renderData
 */
function turnPrimitivesIntoArray(renderData, attributeTypes) {
	const componentProperties = attributeTypes;
	const primitives = ['string', 'number', 'boolean'];
	Object.keys(attributeTypes).forEach((propertyName) => {
		if (
			componentProperties[propertyName].type === 'array' &&
			primitives.indexOf(typeof renderData[propertyName]) !== -1
		) {
			renderData[propertyName] = [renderData[propertyName]];
		}
	});
	return renderData;
}
...
function getComponentRenderData(componentInformation) {
	...
	renderData.children = handlebarsHelperContext.fn ? handlebarsHelperContext.fn() : undefined;
	// Turn primitives into arrays
	// if the schema matches
	if (componentInformation.templateSchema && componentInformation.templateSchema.properties) {
		turnPrimitivesIntoArray(renderData, componentInformation.templateSchema.properties);
	}
	return renderData;
}
