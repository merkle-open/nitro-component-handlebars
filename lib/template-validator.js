...
function getSchema(templateDirectory, schemaName) {
	...
	const oneOfRequiredProperties = [];
	if (parsedSchema.oneOf) {
		parsedSchema.oneOf.forEach((oneOfSchema) => {
			if (oneOfSchema.required) {
				Array.prototype.push.apply(oneOfRequiredProperties, oneOfSchema.required);
			}
		});
	}
	...
	const requiredProperties = _.uniq((parsedSchema.required || []).concat(
		Object.keys(parsedSchema.properties)
			...
			.filter((property) => oneOfRequiredProperties.indexOf(property) === -1)
	));
	const schema = {
